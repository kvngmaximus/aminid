...
import { FileText, Settings, LogOut, Eye, PauseCircle, PlayCircle, Trash2 } from "lucide-react";
...
export default function AdminArticles() {
  ...
  const load = async () => {
    const { data: aRows } = await supabase
      ...
      .limit(200);
    const articleRows = (aRows as Row[]) || [];
    setRows(articleRows);
    const ids = Array.from(new Set(articleRows.map(r => r.author_id)));
    if (ids.length) {
  ...
  const pause = async (id: string) => {
    await supabase.from("articles").update({ status: "paused" }).eq("id", id);
  };
      ...
      <div className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center justify-between mb-8">
            <h1 className="font-poppins font-bold text-3xl text-foreground">Content Moderation</h1>
          </div>

          <div className="bg-white rounded-xl border border-border overflow-hidden">
            <div className="p-6 border-b border-border">All Articles</div>
            <div className="divide-y divide-border">
              {rows.length === 0 ? (
                <div className="p-6 text-muted-foreground">No articles found</div>
              ) : rows.map(r => (
                <div key={r.id} className="p-6 flex items-center justify-between hover:bg-muted/50 transition">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => navigate(`/articles/${r.id}`)} title="View"><Eye className="w-4 h-4" /></Button>
